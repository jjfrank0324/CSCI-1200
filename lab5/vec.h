...
template <class T> void Vec<T>::print_V(){
  std::cout << "The m_size is:" << m_size << " ;\n";
  std::cout << "The m_alloc is:" << m_alloc << " ;\n";
  std::cout << "The m_data is: \n";
  for (int i = 0; i<m_size; ++i )
    std::cout << " " << this->m_data[i];
  std::cout << std::endl << std::endl;
}
template <class T> void Vec<T>::smaller(){

  while(((m_size)*2)<=m_alloc){
...
template <class T> int remove_matching_elements(Vec<T>& v,T value){
  typename Vec<T>::iterator P = v.begin();
  int count =0;
  while( P != v.end()){
    if (*P == value){
      v.erase(P);
