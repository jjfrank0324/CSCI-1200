...
// =======================================================================
/*
In the student test, I did several test for the ShipFreight functions,
I tried  several conrner cases to see will this work normally or not.
the specific description for each cases are all on the Cout
*/
void StudentTests() {
  ...
  std::cout << "STUDENT TESTS" << std::endl;
    ...
    std::cout<<"Case #1 for ShipFreight: Overweight Train! "<<std::endl;
    std::cout<<"To see weather the train will add overweight Freight or not "<<std::endl;
    ...
    TrainCar* all_freight = NULL;
    for (int i = 0; i < 20; i++) {
      PushBack(all_freight, TrainCar::MakeFreightCar(400));
    }

    std::vector<TrainCar*> trains = ShipFreight(all_engines, all_freight, 65,10);
    ...
    if (all_freight != NULL) {
      std::cout << "Now is printing out of Freight:" << std::endl;
      SanityCheck(all_freight);
      PrintTrain(all_freight);
    }

    //print out the outputs of the trains to see we are right are not
    for (unsigned int i = 0; i < trains.size(); i++) {
      std::cout<<"Now is pringting out of all trains "<<std::endl;
      SanityCheck(trains[i]);
      PrintTrain(trains[i]);
    }

    // mkae sure that all the train are deleted
    DeleteAllCars(all_engines);
    DeleteAllCars(all_freight);
    for (unsigned int i = 0; i < trains.size(); i++) {
      DeleteAllCars(trains[i]);
    ...
    std::cout<<"Case #2 for ShipFreight: constant Frieight Train! "<<std::endl;
    std::cout<<"To see the consstence of the adding function"<<std::endl;
    std::cout<<"Will the output be consistence for each train?"<<std::endl;

    TrainCar* ENGINES = NULL;
    for (int i = 0; i < 10; i++) {
      PushBack(ENGINES, TrainCar::MakeEngine());
    }
    TrainCar* FREIGHT = NULL;
    for (int i = 0; i < 24; i++) {
      PushBack(FREIGHT, TrainCar::MakeFreightCar(33));
    }

    std::vector<TrainCar*> TRAINS = ShipFreight(ENGINES, FREIGHT, 65,10);
    ...
    std::cout<< "now staring making engine test"<<std::endl;
    TrainCar* t;



    std::cout<< "#1 Case one Lots of engines and randomly arrged Dining Car!"<<std::endl;
    ...
    std::cout<< "#2 Case two, Lots of others Cars but only two engines"<<std::endl;
    t = NULL;
    PushBack(t, TrainCar::MakePassengerCar());
    PushBack(t, TrainCar::MakeDiningCar());
    PushBack(t, TrainCar::MakeSleepingCar());
    ...
    std::cout<< "#3 all engine to the one side "<<std::endl;
    t = NULL;
